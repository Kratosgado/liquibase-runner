...
import { getExecutionStrategy, getProjectOverrides } from './configManager.js';
...
async function detectProject( rootPath: string ): Promise<LiquibaseProject | undefined> {
	const detected = await detectStrategy( rootPath );
	...
	// Apply configured/resolved strategy (supports user setting 'auto' to detect)
	const resolvedStrategy = resolveStrategyFromConfig( configuredStrategy, detected );

	// Apply per-project overrides from workspace settings
	const overrides = getProjectOverrides( rootPath );
	const finalChangelog = overrides.changelogFile ?? changelogFile;
	const finalProperties = overrides.propertiesFile ?? propertiesFile;
	const finalResolved = ( overrides.resolvedStrategy as 'maven' | 'gradle' | 'cli' ) ?? resolvedStrategy;
	...
	return {
		...
		resolvedStrategy: finalResolved,
		changelogFile: finalChangelog,
		propertiesFile: finalProperties,
	};
}
