...
function generateForm( project: LiquibaseProject, conns: DatabaseConnection[], saved: Partial<FormSubmitResult> ) {
	const outputFile = h( saved.outputFile ?? '' );
	const refUrl = h( saved.referenceUrl ?? '' );
...
<div class="field">
  ...
  <div class="row">
    <input type="text" id="outputFile" value="${outputFile}" placeholder="src/main/resources/db/changelog/migrations/YYYYMMDD.yaml" />
    <button type="button" id="btnBrowseOutput">Browse…</button>
  </div>
  <p id="sqlHint" class="hint sql-hint hidden">SQL changelogs require a database type in the name: <code>*.postgresql.sql</code>, <code>*.mysql.sql</code>, etc.</p>
  <p id="sqlError" class="hint sql-error hidden">Invalid SQL filename — use <code>*.{databaseType}.sql</code> (e.g. <code>generated.postgresql.sql</code>).</p>
</div>
<div class="field">
  <label>Contexts</label>
  <input type="text" id="contexts" value="${contexts}" placeholder="e.g. dev,test" />
...
function onOutputFileChange() {
  const f = val('outputFile');
  const isSql = f.toLowerCase().endsWith('.sql');
  const parts = f.toLowerCase().split('.');
  const isValid = parts.length >= 3 && parts[parts.length - 1] === 'sql';
  document.getElementById('sqlHint').classList.toggle('hidden', !isSql || isValid);
  document.getElementById('sqlError').classList.add('hidden');
}
function submitForm(action) {
  const f = val('outputFile');
  const fParts = f.toLowerCase().split('.');
  if (f.toLowerCase().endsWith('.sql') && !(fParts.length >= 3 && fParts[fParts.length - 1] === 'sql')) {
    document.getElementById('sqlError').classList.remove('hidden');
    document.getElementById('sqlHint').classList.add('hidden');
    return;
  }
...
document.getElementById('outputFile').addEventListener('input', onOutputFileChange);
document.getElementById('btnAddConn').addEventListener('click', addConn);
document.getElementById('btnBrowseOutput').addEventListener('click', () => browse('outputFile'));
